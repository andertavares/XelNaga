...
UnitGroupManager::UnitGroupManager()
{
	...
	for(BWAPI::Unitset::iterator i=BWAPI::Broodwar->getAllUnits().begin();i!=BWAPI::Broodwar->getAllUnits().end();i++)
	{
		onUnitDiscover(*i);
	}
	neutral=NULL;
	for(BWAPI::Playerset::iterator i=BWAPI::Broodwar->getPlayers().begin();i!=BWAPI::Broodwar->getPlayers().end();i++)
	{
		if ((*i)->isNeutral())
			neutral=*i;
	}
